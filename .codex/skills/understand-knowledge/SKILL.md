...
5. Move the intermediate directory into a timestamped trash location. Do not recursively delete it; this project requires manual deletion:
   ```bash
   ...
   if [ -n "$TARGET_DIR" ] && [ -d "$UA_DIR/intermediate" ]; then
     mv "$UA_DIR/intermediate" "$UA_DIR/.trash-knowledge-$(date +%s)"
   fi
   ```
