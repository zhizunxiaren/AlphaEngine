...
1. Move intermediate files **except** `scan-manifest.json` into a timestamped trash directory. Do not recursively delete them; this project requires manual deletion:
   ```bash
   INTER="$UA_DIR/intermediate"
   TRASH="$UA_DIR/.trash-figma-$(date +%s)"
   mkdir -p "$TRASH"
   find "$INTER" -mindepth 1 -maxdepth 1 -not -name 'scan-manifest.json' -exec mv {} "$TRASH/" \;
   ```
