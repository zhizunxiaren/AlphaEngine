...
## [2026-09-21] knowledge + outcomes | 随机源显式化与多线程渲染落地，复核蒙特卡洛前提

**实现推进（本轮一次提交）**：① 随机数架构由全局 `random_double()` 改为**显式注入的 `rng_t` 上下文**（`mt19937` + 均匀分布），沿 `main` 场景构建 → `camera::get_ray` → `material::scatter` → 递归全程传递，`vec3::random` / `random_unit_vector` / `random_in_unit_disk` / `random_on_hemisphere` 一律改为接 `rng_t&`，代码中不再存在隐藏全局随机状态；② **多线程渲染**：按行分块、每线程持有独立引擎，写入共享帧缓冲后由主线程顺序输出 PPM，并以 `std::chrono::steady_clock` 统计渲染耗时；③ `material::scatter` 由「返回 false 的空实现」升级为**纯虚接口**；④ `main.cpp` 接入 RTIOW 终局场景（22×22 随机球阵列 + 三个大球，`image_width` 1200 / `sample_per_pixel` 100 / `defocus_angle` 0.6 / `focus_dist` 10），场景 RNG 使用固定种子以便对比渲染参数。编译与运行证据：Release 产物时间晚于全部源码改动，且运行后写出了 `image.ppm`。

**新增结论（写入知识层）**：[[20-Knowledge/Concepts/路径追踪与蒙特卡洛积分]] 第 5.2 节新增第 4 点与一处专门提示——**线程化不改变估计器性质**：同一像素的 `N` 个样本仍由同一条工作线程按序产生，像素间不共享随机序列，故第 2 节的独立同分布前提、无偏性与 `σ/√N` 方差结论均不受影响；受影响的只是**可复现性**，因为种子按线程序号派生，噪声图案随线程数变化。进一步指出一个设计张力：**「动态行调度」与「每线程固定种子」互斥**——动态调度会让同一线程处理的像素集合随运行变化，从而重新引入不确定性；要两者兼得，应把种子改为按行号或像素坐标派生。该结论对后续任何并行渲染改造都适用，故写入知识层而非代码注释。

**同步**：[[10-Outcomes/10-AlphaEngine]] 实现现状表新增「随机数架构」行，能力行补入多线程渲染与耗时统计，已知差距行补入「静态行分块（动态调度未实现，`<atomic>` 已包含未使用）」与「噪声图案不跨机器可复现」，事实截点更新为 2026-09-21。[[20-Knowledge/Concepts/光照模型]] 无需改动：本轮未触及着色模型，其第 8.1 节对「无直接光项」的判断仍然成立。

**代码评审发现（未修改，待作者处理）**：`camera.h` 分块处的注释称「行级动态调度……动态取行比静态分块更能自动负载均衡」，而实现是 `rows_per_thread` 的**静态连续分块**，注释描述的是尚未实现的方案；`rtweekend.h` 包含的 `<atomic>` 在整个 `Engine/` 中无任何使用点，与被注释描述的动态调度器是同一处未完成痕迹。两处均属"意图已写明、实现未落地"，因此按事实登记，不擅自改动代码。
